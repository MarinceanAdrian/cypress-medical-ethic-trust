...
describe("Moonlysoft", () => {
  ...
  it.skip("Create new clients", () => {
    ...
    cy.get("a.MuiButtonBase-root h6", { timeout: 10000 })
      .contains("Clienti")
      .click();
    ...
    cy.get("[role='presentation']").contains("CAM Manager").click();
  ...
  it("Check newly create client", () => {
    ...
    cy.get("#start-adornment-email").type(newClient["company-name"]);
    cy.wait(2000);
    cy.get(".MuiTableBody-root").then((tableBody) => {
      cy.wrap(tableBody).should("contain", newClient["company-name"]);
      cy.wrap(tableBody).should("contain", newClient["cui"]);
      // cy.wrap(tableBody).should("contain", newClient["consultant"]);
    });
  });
  ...
  it("Check modified client", () => {
    ...
    cy.wait(2000);
    cy.get(".MuiTableBody-root").then((tableBody) => {
      cy.wrap(tableBody).should("contain", newClient.updatedCompanyName);
      cy.wrap(tableBody).should("contain", newClient["cui"]);
      // cy.wrap(tableBody).should("contain", newClient["consultant"]);
    });
  });
